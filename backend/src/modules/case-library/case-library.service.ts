...
export class CaseLibraryService {
  constructor(
    ...
    private readonly repository: Repository<CaseLibrary>,
  ) {}

  async create(data: Partial<CaseLibrary>): Promise<CaseLibrary> {
  ...
  async findAll(query: any): Promise<PaginationResultDto<CaseLibrary>> {
  ...
  async findOne(id: number): Promise<CaseLibrary> {
  ...
  async update(id: number, data: Partial<CaseLibrary>): Promise<CaseLibrary> {
